...
var ASM_CONSTS = {
  36424: function() {return Module['suspended'];},  
 36456: function($0) {Module['suspended'] = $0;},  
 36486: function() {var len = lengthBytesUTF8(tx) + 1; var s = _malloc(len); stringToUTF8(tx, s, len); return s;},  
 36583: function() {tx = "";},  
 36596: function($0, $1) {const classList = document.querySelector('#btn' + $0).classList; const highlight = 'highlight'; $1 ? classList.add(highlight) : classList.remove(highlight);},  
 36757: function($0) {const date = new Date(Date.now() + $0); return date.getSeconds() | (date.getMinutes() << 6) | (date.getHours() << 12) | (date.getDate() << 17) | ((date.getMonth() + 1) << 22) | ((date.getFullYear() - 2020) << 26);},  
 36975: function() {document.getElementById("custom").style.display = "none";},  
 37035: function() {document.getElementById("classic").style.display = "none";},  
 37096: function() {document.getElementById("gshock").style.display = "";},  
 37152: function() {setNewSkin("dw5600");},  
 37178: function() {document.getElementById("classic").style.display = "none";},  
 37239: function() {document.getElementById("custom").style.display = "none";},  
 37299: function() {document.getElementById("gshock").style.display = "none";},  
 37359: function($0, $1) {document.querySelectorAll("[data-com='" + $0 + "'][data-seg='" + $1 + "']") .forEach((e) => e.style.opacity = 1);},  
 37477: function($0, $1) {document.querySelectorAll("[data-com='" + $0 + "'][data-seg='" + $1 + "']") .forEach((e) => e.style.opacity = 0);},  
 37595: function() {document.querySelectorAll("[data-com][data-seg]") .forEach((e) => e.style.opacity = 0);},  
 37687: function() {if (!Module['audioContext']) { Module['audioContext'] = new (window.AudioContext || window.webkitAudioContext)(); }},  
 37807: function($0) {const audioContext = Module['audioContext']; if (!audioContext) return; if (!(audioContext._oscillator && audioContext._gain)) { const oscillator = audioContext.createOscillator(); const gain = audioContext.createGain(); oscillator.type = 'triangle'; oscillator.connect(gain); gain.connect(audioContext.destination); oscillator.start(0); audioContext._oscillator = oscillator; audioContext._gain = gain; } audioContext._oscillator.frequency.value = 1e6/$0; audioContext._gain.gain.value = volumeGain;},  
 38312: function() {const audioContext = Module['audioContext']; if (audioContext && audioContext._gain) { audioContext._gain.gain.value = 0; }},  
 38440: function($0, $1, $2) {let filter = document.getElementById("ledcolor"); let color_matrix = filter.children[0].values.baseVal; color_matrix[0].value = $0 / 255; color_matrix[6].value = $1 / 255; color_matrix[12].value = $2 / 255; document.getElementById('light').style.opacity = Math.min(255, $0 + $1 + $2) / 255;},  
 38735: function() {return lat;},  
 38751: function() {return lon;},  
 38767: function() {return temp_c || 25.0;}
};
...
var _vectors = Module['_vectors'] = 36256;
